# `ex1_main.py` 代码逐行详细解释

> 本文档按主函数（`if __name__ == "__main__":` 块）的运行顺序，对 `ex1_main.py` 中的**每一行代码**进行详细解释，包括所有类方法、辅助函数和工具方法。  
> 涉及计算的代码给出对应的数学公式，行号在前，公式在后，分段对应。  
> 所有公式和行内符号均使用 LaTeX 渲染：块级公式用 `\[ ... \]`，行内符号用 `\( ... \)`。

---

## 目录

1. [模块导入（第 31–43 行）](#1-模块导入第-3143-行)
2. [顶层辅助函数 `create_grid`（第 46–79 行）](#2-顶层辅助函数-create_grid第-4679-行)
3. [`Example1Model` 类定义（第 82–88 行）](#3-example1model-类定义第-8288-行)
4. [`__init__` 构造函数（第 90–93 行）](#4-__init__-构造函数第-9093-行)
5. [`_set_fields`：字段初始化（第 1199–1234 行）](#5-_set_fields字段初始化第-11991234-行)
6. [主函数入口与参数配置（第 1305–1327 行）](#6-主函数入口与参数配置第-13051327-行)
7. [网格循环前的初始化（第 1328–1333 行）](#7-网格循环前的初始化第-13281333-行)
8. [细化等级主循环——建立网格（第 1335–1345 行）](#8-细化等级主循环建立网格第-13351345-行)
9. [模型实例化（第 1347–1348 行）](#9-模型实例化第-13471348-行)
10. [运行时间依赖模型（第 1350 行）](#10-运行时间依赖模型第-1350-行)
11. [`prepare_simulation`（第 738–751 行）](#11-prepare_simulation第-738751-行)
    - [11.1 `create_grid` 方法（第 95–113 行）](#111-create_grid-方法第-95113-行)
    - [11.2 `_set_time_parameters`（第 1154–1168 行）](#112-_set_time_parameters第-11541168-行)
    - [11.3 `_set_rock_and_fluid`（第 442–448 行）](#113-_set_rock_and_fluid第-442448-行)
    - [11.4 `_initial_condition`（第 1112–1141 行）](#114-_initial_condition第-11121141-行)
    - [11.5 `_update_all_apertures`（第 290–387 行）](#115-_update_all_apertures第-290387-行)
    - [11.6 `_initial_scalar`（第 1143–1149 行）](#116-_initial_scalar第-11431149-行)
    - [11.7 `_initial_temperature`（第 1151–1152 行）](#117-_initial_temperature第-11511152-行)
    - [11.8 `_set_parameters` 总流程](#118-_set_parameters-总流程)
    - [11.9 `_set_mechanics_parameters`（第 450–521 行）](#119-_set_mechanics_parameters第-450521-行)
    - [11.10 `_set_scalar_parameters`（第 523–577 行）](#1110-_set_scalar_parameters第-523577-行)
    - [11.11 `_set_vector_source`（第 579–609 行）](#1111-_set_vector_source第-579609-行)
    - [11.12 `_set_permeability_from_aperture`（第 389–440 行）](#1112-_set_permeability_from_aperture第-389440-行)
    - [11.13 `_set_temperature_parameters`（第 611–704 行）](#1113-_set_temperature_parameters第-611704-行)
    - [11.14 `_dVdt_source`（第 706–720 行）](#1114-_dvdt_source第-706720-行)
    - [11.15 `_assign_discretizations`（第 722–736 行）](#1115-_assign_discretizations第-722736-行)
    - [11.16 `_set_exporter`（第 950–954 行）](#1116-_set_exporter第-950954-行)
    - [11.17 `_export_step`（第 956–1021 行）](#1117-_export_step第-9561021-行)
12. [时间推进与 Newton 迭代](#12-时间推进与-newton-迭代)
    - [12.1 `before_newton_loop`（第 753–755 行）](#121-before_newton_loop第-753755-行)
    - [12.2 `before_newton_iteration`（第 757–782 行）](#122-before_newton_iteration第-757782-行)
    - [12.3 `assemble_and_solve_linear_system`（第 796–817 行）](#123-assemble_and_solve_linear_system第-796817-行)
    - [12.4 `check_convergence`（第 819–948 行）](#124-check_convergence第-819948-行)
    - [12.5 `after_newton_convergence`（第 786–794 行）](#125-after_newton_convergence第-786794-行)
    - [12.6 `_adjust_time_step`（第 1178–1197 行）](#126-_adjust_time_step第-11781197-行)
    - [12.7 `_save_data`（第 1030–1110 行）](#127-_save_data第-10301110-行)
    - [12.8 `_export_pvd`（第 1023–1028 行）](#128-_export_pvd第-10231028-行)
13. [导出与数据保存（第 1353–1355 行）](#13-导出与数据保存第-13531355-行)
14. [多网格精化后处理（第 1357–1360 行）](#14-多网格精化后处理第-13571360-行)
15. [辅助类：`Water`（第 1249–1283 行）](#15-辅助类water第-12491283-行)
16. [辅助类：`Granite`（第 1286–1302 行）](#16-辅助类granite第-12861302-行)
17. [辅助方法：`_variable_increment`（第 1236–1246 行）](#17-辅助方法_variable_increment第-12361246-行)
18. [辅助方法：`_porosity`（第 115–119 行）](#18-辅助方法_porosity第-115119-行)
19. [辅助方法：`_aperture`（第 272–279 行）](#19-辅助方法_aperture第-272279-行)
20. [辅助方法：`_specific_volumes`（第 281–288 行）](#20-辅助方法_specific_volumes第-281288-行)
21. [辅助方法：`_fluid_density`（第 212–236 行）](#21-辅助方法_fluid_density第-212236-行)
22. [辅助方法：`_biot_alpha`（第 184–188 行）](#22-辅助方法_biot_alpha第-184188-行)
23. [辅助方法：`_biot_beta`（第 190–210 行）](#23-辅助方法_biot_beta第-190210-行)
24. [辅助方法：`_set_friction_coefficient`（第 1170–1176 行）](#24-辅助方法_set_friction_coefficient第-11701176-行)
25. [边界条件方法（第 121–182 行）](#25-边界条件方法第-121182-行)
26. [耦合系数方法（第 238–270 行）](#26-耦合系数方法第-238270-行)
